...
class VisionTests(unittest.TestCase):
    def test_opencv5_guard_is_explicit(self):
        with self.assertRaises(VisionError):
            require_opencv5(version="4.13.0")
    ...
    def test_synthetic_defect_roundtrip(self):
        ...
        packet = inspect_pair(
            encode_png(reference),
            encode_png(inspection),
            config=InspectionConfig(min_region_area_px=40, min_delta=32, max_regions=16),
    ...
    def test_dimension_mismatch_fails_closed(self):
        ...
        with self.assertRaises(VisionError):
            inspect_pair(encode_png(reference), encode_png(inspection), allow_opencv4_dev=True)
