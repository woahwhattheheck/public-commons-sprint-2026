...
Resource-exhaustion bounds apply to both supported input modes. Byte-parsed JSON is capped by the retained input-size and parser guards. Direct Python-object compile/verify calls are structurally bounded before the analyzer walks them: `changes`, ADR rows and runbook rows are limited to 5,000 each; per-file `tests` and per-document `covers` are limited to 500 references per row; each nested reference family is limited to 20,000 references in aggregate. These complement the retained 5,000-file and 20,000-dependency ceilings. The direct-object caps intentionally bound work rather than pretending an in-memory object has the CLI's exact serialized byte ceiling.
