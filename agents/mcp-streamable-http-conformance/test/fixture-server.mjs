...
export async function startFixture(options = {}) {
  ...
  async function handle(req, res) {
    ...
    if (msg.method === 'tools/list') {
      if (options.oversized) return sendRpc(res, 200, { jsonrpc: '2.0', id: msg.id, result: { tools: [{ name: 'safe_probe_fixture', description: 'x'.repeat(options.oversized) }] } });
      return sendRpc(res, 200, { jsonrpc: '2.0', id: msg.id, result: { tools: options.emptyTools ? [] : [{ name: 'safe_probe_fixture' }] } });
    }
    if (msg.method === 'resources/list') return sendRpc(res, 200, { jsonrpc: '2.0', id: msg.id, result: { resources: [{ uri: 'ui://fixture' }] } });
    ...
    return sendRpc(res, 200, { jsonrpc: '2.0', id: msg.id, error: { code, message } });
  }
