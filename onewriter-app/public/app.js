...
function renderLanes() {
  ...
  host.innerHTML = state.lanes.map((lane) => {
    ...
    return `<article class="lane-card">
      ...
      ${lane.effective_refence_pending ? '<p class="callout warning">One-shot human lease expired; effective prior fence shown. Next mutation will persist the refence.</p>' : ""}
    </article>`;
  }).join("");
}
...
$("#claim-form").addEventListener("submit", async (event) => {
  ...
  showResult("#claim-result", "Requesting atomic lease…");
  try {
...
$("#event-form").addEventListener("submit", async (event) => {
  event.preventDefault();
  showResult("#event-result", "Recording evidence-bound transition…");
  try {
...
refresh().catch((error) => {
  $("#server-time").textContent = "state unavailable";
  console.error(error);
});
